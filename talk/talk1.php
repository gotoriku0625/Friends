...
if(isset($_POST['submit'])&&$_POST['submit']==="送信"){//submitが存在し、かつ中身が送信であるか
    // $talk=[];
    // $talk["person"]="person1";
    // $talk["imgPath"]="#";
    ...
    // 入力値格納処理
    if(){
        $insert='insert into talk (sender_id,receber_id,content) values(:sender_id,:receber_id,:content)';
        $sql = $pdo->prepare($insert);
        $sql->bindParam(':sender_id',$)
    // }else{
    //     // ファイルがない場合、新規作成処理
